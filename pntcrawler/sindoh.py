...
def get_sindoh_cm3091(dept, model, ip, chrome_driver_path):
    """
    Sindoh CM3091(D422) 및 DM6011(D722) 모델의 소모품 정보를 Dictionary로 반환합니다.
    ...
    try:
        ...
        # class="LevelPer LevelPer1"인 input 태그를 전부 가져옴
        levelPer1 = soup.findAll('input', {'class': 'LevelPer LevelPer1'})
        ...
        data = get_crawl_data(dept=dept, model=model, ip=ip,
                              # input 태그의 value 속성 값을 가져옴
                              toner_y=int(levelPer1[0]['value']),
                              toner_m=int(levelPer2[0]['value']),
                              toner_c=int(levelPer3[0]['value']),
                              toner_k=int(levelPer4[0]['value']),
                              # 이상하게 드럼 값은 class="levelPer LevelPer1"인 태그 쪽에 몰려있음...
                              drum_c=int(levelPer1[1]['value']),
                              drum_m=int(levelPer1[2]['value']),
                              drum_y=int(levelPer1[3]['value']),
...
def get_sindoh_d417(dept, model, ip, chrome_driver_path):
    """
    Sindoh D417, D716 및 CM3091(D422) 일부 모델의 소모품 정보를 Dictionary로 반환합니다.
    ...
    try:
        ...
        data = get_crawl_data(dept=dept, model=model, ip=ip,
                              toner_y=int(re.findall(
                                  r"\d+", data[0].getText())[0]),
                              toner_m=int(re.findall(
                                  r"\d+", data[1].getText())[0]),
                              toner_c=int(re.findall(
                                  r"\d+", data[2].getText())[0]),
                              toner_k=int(re.findall(
                                  r"\d+", data[3].getText())[0]),
                              drum_c=int(re.findall(
                                  r"\d+", data[4].getText())[0]),
                              drum_m=int(re.findall(
                                  r"\d+", data[5].getText())[0]),
                              drum_y=int(re.findall(
                                  r"\d+", data[6].getText())[0]),
...
def get_sindoh_b605n(dept, model, ip, chrome_driver_path):
    """
